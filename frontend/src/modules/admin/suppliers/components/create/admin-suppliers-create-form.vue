<template>
	<div class="flex flex-col gap-6 mx-auto w-full md:w-2/3">
		<Card>
			<CardHeader>
				<CardTitle>Создать Поставщика</CardTitle>
				<CardDescription>Заполните форму ниже, чтобы создать нового поставщика.</CardDescription>
			</CardHeader>
			<CardContent>
				<form
					@submit="submitForm"
...
const emit = defineEmits<{
	(e: 'onSubmit', formValues: CreateSupplierDTO): void
	(e: 'onCancel'): void
}>()
...
const { handleSubmit } = useForm<CreateSupplierDTO>({
