<template>
	...
	>
		...
		>
			<DialogHeader>
				<DialogTitle>Размер для принтера штрих-кодов</DialogTitle>
				<DialogDescription>
					...
					<FormItem>
						...
						>
							...
							<SelectContent>
								<SelectGroup>
									<SelectItem
										v-for="option in PREDEFINED_BARCODE_SIZES"
										:key="option.label"
										:value="option.label"
									>
										{{ option.label }}
									...
									<!-- Свой размер -->
									<SelectItem :value="BarcodePrinterSizeName.CUSTOM">
										{{ BarcodePrinterSizeName.CUSTOM }}
									</SelectItem>
								</SelectGroup>
							</SelectContent>
						</Select>
...
import { BarcodePrinterSizeName, PREDEFINED_BARCODE_SIZES, SAVED_BARISTA_BARCODE_SETTINGS_KEY } from '@/core/constants/barcodes.constants'
import { toTypedSchema } from '@vee-validate/zod'
import { Settings } from 'lucide-vue-next'

/**
 * ENUM: Локальные ключи, которые используем в localStorage
 */
...
const formSchema = z.object({
  standard: z.nativeEnum(BarcodePrinterSizeName),
  width: z.preprocess((val) => parseFloat(val as string), z.number().min(1)),
  height: z.preprocess((val) => parseFloat(val as string), z.number().min(1)),
});
...
const INITIAL_VALUES = {
  standard: BarcodePrinterSizeName.SIZE_100x50,
  width: 100,
  height: 50,
};
 ...
 */
const isCustomSelected = computed(() => values.standard === BarcodePrinterSizeName.CUSTOM);
...
function loadFromLocalStorage() {
  ...
  if (stored) {
    ...
    // Если ничего не сохранено, сбрасываем на начальные значения
 ...
 * Если пользователь выбрал один из предопределённых размеров,
 * мы автоматически задаём ширину/высоту.
 */
function onStandardChange(selected: string) {
  if (selected !== BarcodePrinterSizeName.CUSTOM) {
    const found = PREDEFINED_BARCODE_SIZES.find((size) => size.label === selected);
    if (found) {
      setValues({
    ...
    // Пользовательский размер: сбрасываем на что-то разумное
    setValues({
      standard: BarcodePrinterSizeName.CUSTOM,
      width: 50,
      height: 25,
    });
  }
...
const onSubmit = handleSubmit((values) => {
  localStorage.setItem(SAVED_BARISTA_BARCODE_SETTINGS_KEY, JSON.stringify(values));
  toast({
    title: "Успех",
