<template>
	...
	>
		<DialogContent :include-close-button="false">
			<DialogHeader>
				<DialogTitle>Выберите доступный модификатор</DialogTitle>
			</DialogHeader>
...
const { data: additives, refetch } = useQuery({
  queryKey: computed(() => [
  'admin-available-additives',
  filter.value
]),
  queryFn: () => storeAdditivesService.getAvailableToAddAdditivesList(filter.value),
})
