...
import type { ProductCategoryTranslationsDTO } from '@/modules/kiosk/products/models/product.model'
...
const { data: translations, isLoading: isLoadingTranslations } = useQuery({
  queryKey: computed(() => ['product-category-translations', categoryId.value]),
  queryFn: () => productsService.getProductCategoryTranslations(categoryId.value),
...
const { mutate: saveTranslations, isPending: isSaving } = useMutation({
  mutationFn: (dto: ProductCategoryTranslationsDTO) => productsService.upsertProductCategoryTranslations(categoryId.value, dto),
  onMutate: () => {
    toast({
  ...
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['product-category-translations', categoryId.value] })
    toast({
      title: 'Успех!',
...
function handleSubmit(
  ...
  const dto: ProductCategoryTranslationsDTO = {}

  payload.forEach((item) => {
    const [field, locales] = Object.entries(item)[0]
    dto[field as keyof ProductCategoryTranslationsDTO] = locales
  })
