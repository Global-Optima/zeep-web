...
const createMutation = useMutation({
	...
	onMutate: () => {
		toast({
			title: 'Создание...',
			description: 'Добавление новых товаров магазина. Пожалуйста, подождите.',
		})
	},
	onSuccess: () => {
		...
		toast({
			title: 'Успех!',
			description: 'Товары магазина успешно добавлены.',
		})
		router.back()
	},
	onError: () => {
		toast({
			title: 'Ошибка',
			description: 'Произошла ошибка при добавлении товаров магазина.',
			variant: 'destructive',
		})
	},
})
...
function handleCreate(dto: CreateStoreProductDTO[]) {
	if (dto.length === 0) {
		toast({
			title: 'Ошибка',
			description: 'Список товаров пуст. Пожалуйста, добавьте товары перед сохранением.',
			variant: 'destructive',
		})
		return
	}
