...
func (r *storeProvisionRepository) DeductProvisionVolume(tx *gorm.DB, storeID, provisionID uint, requiredVolume float64) ([]uint, error) {
	if requiredVolume <= 0 {
		return nil, nil
	}
	...
	for _, provision := range provisions {
		...
		if provision.Volume == 0 {
			depletedIDs = append(depletedIDs, provision.ID)
		}

		if err := tx.Model(&data.StoreProvision{}).Where("id = ?", provision.ID).Update("volume", provision.Volume).Error; err != nil {
			return nil, fmt.Errorf("failed to update provision volume for ID %d: %w", provision.ID, err)
		}
	}

	if remaining > 0 {
		return nil, fmt.Errorf("%w: not enough provision volume for provision ID %d", types.ErrInsufficientStoreProvision, provisionID)
	}

	return depletedIDs, nil
}
