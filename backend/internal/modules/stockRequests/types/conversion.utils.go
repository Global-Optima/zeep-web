...
func getAutogeneratedComments(details []StockRequestDetails) *localization.LocalizedMessages {
	...
	return CombineComments(mismatchComments, unexpectedComments)
}
...
func convertDetailsToStockRequestDetails(details []datatypes.JSON) []StockRequestDetails {
	...
	for _, jsonDetail := range details {
		var detail StockRequestDetails
		if err := json.Unmarshal(jsonDetail, &detail); err == nil {
			stockRequestDetails = append(stockRequestDetails, detail)
		}
	}
