...
-- Preventing unit soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_unit_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
DECLARE
rec record;
    table_has_deleted boolean;
    sql_check text;
BEGIN
        ...
        IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
            ...
            LOOP
                SELECT EXISTS(
                    SELECT 1
                    FROM pg_attribute
                    WHERE attrelid = rec.child_table::regclass
                        AND attname = 'deleted_at'
                        AND NOT attisdropped
                )
                INTO table_has_deleted;

                IF table_has_deleted THEN
                    sql_check := format('
                        SELECT 1 FROM %I
                        WHERE unit_id = $1
                ...
                EXECUTE sql_check INTO table_has_deleted USING OLD.id;

                IF table_has_deleted THEN
                    RAISE EXCEPTION
                        'Cannot soft-delete Unit % because it is in use by table %', OLD.id, rec.child_table
                        USING ERRCODE = 'P6001';
                END IF;
            END LOOP;
        END IF;
...
CREATE TRIGGER prevent_unit_soft_delete_if_in_use
    BEFORE UPDATE ON units
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_unit_soft_delete_if_in_use();

-- Preventing ingredient soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_ingredient_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
DECLARE
rec record;
    table_has_deleted boolean;
...
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
            ...
            IF table_has_deleted THEN
                RAISE EXCEPTION
                    'Cannot soft-delete Ingredient % because it is in use by table %', OLD.id, rec.child_table
                        USING ERRCODE = 'P6002';
            END IF;
            END LOOP;
        END IF;
...
CREATE TRIGGER prevent_ingredient_soft_delete_if_in_use
    BEFORE UPDATE ON ingredients
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_ingredient_soft_delete_if_in_use();

-- Preventing ingredient categories soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_ingredient_category_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      IF EXISTS (
        ...
        RAISE EXCEPTION
          'Cannot soft-delete Ingredient category % because it is still in use by ingredients', OLD.id
          USING ERRCODE = 'P6003';
END IF;
END IF;
END IF;
...
CREATE TRIGGER prevent_ingredient_category_soft_delete_if_in_use
    BEFORE UPDATE ON units
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_ingredient_category_soft_delete_if_in_use();

-- Preventing product categories soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_product_category_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      IF EXISTS (
        ...
        RAISE EXCEPTION
          'Cannot soft-delete Product category % because it is still in use by products', OLD.id
          USING ERRCODE = 'P6004';
      END IF;
    END IF;
  END IF;
...
CREATE TRIGGER prevent_product_category_soft_delete_if_in_use
    BEFORE UPDATE ON units
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_product_category_soft_delete_if_in_use();

-- Preventing additive categories soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_additive_category_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      IF EXISTS (
        SELECT 1
        FROM additives c
        WHERE c.additive_category_id = OLD.id
          AND c.deleted_at IS NULL
      ) THEN
        RAISE EXCEPTION
          'Cannot soft-delete Additive category % because it is still in use by additives', OLD.id
          USING ERRCODE = 'P6005';
END IF;
END IF;
END IF;
...
CREATE TRIGGER prevent_additive_category_soft_delete_if_in_use
    BEFORE UPDATE ON units
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_additive_category_soft_delete_if_in_use();

-- Preventing stock material categories soft deleting if in use
CREATE OR REPLACE FUNCTION prevent_stock_material_category_soft_delete_if_in_use()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    IF OLD.deleted_at IS NULL AND NEW.deleted_at IS NOT NULL THEN
      IF EXISTS (
        SELECT 1
        FROM stock_materials c
        WHERE c.category_id = OLD.id
          AND c.deleted_at IS NULL
      ) THEN
        RAISE EXCEPTION
          'Cannot soft-delete Stock material category % because it is still in use by stock_materials', OLD.id
          USING ERRCODE = 'P6006';
END IF;
END IF;
END IF;
...
CREATE TRIGGER prevent_stock_material_category_soft_delete_if_in_use
    BEFORE UPDATE ON units
    FOR EACH ROW
    EXECUTE PROCEDURE prevent_stock_material_category_soft_delete_if_in_use();